...
def main():
    ...
    parser.add_argument('--title', default="Create a Budget\nfor You")
    ...
    if args.theme == 'dark':
        bg_color = (25, 28, 35)      # Dark grey-blue background
        hill_color = (35, 42, 53)    # Slightly lighter dark grey for the hill
        text_color = (255, 255, 255, 255)
    else:
        bg_color = BG_COLOR          # Default light green
    ...
    # Draw hill using theme color
    bbox = (-500, 1850, CANVAS_SIZE[0] + 500, CANVAS_SIZE[1] + 800)
    draw.ellipse(bbox, fill=hill_color)
    
    title_font = get_font(110)
    draw_text_centered(draw, args.title.replace('\\n', '\n'), 120, title_font, fill=text_color)
    
    draw_category_icons(canvas)
    
    if os.path.exists(args.input):
        raw_img = Image.open(args.input).convert("RGBA")
        w, h = raw_img.size
        
        # Standard screenshot is 1080x2400. We crop out the status bar (top 130) and the bottom nav (bottom 250)
        # to fit inside the device frame roughly.
        cropped = raw_img.crop((0, 130, w, h - 250))
        
        # Resize to fit device
        cropped = cropped.resize(DEVICE_SIZE, Image.Resampling.LANCZOS)
        cropped = mask_rounded(cropped, 60)
        ...
        canvas.paste(cropped, (device_x, DEVICE_Y), cropped)
    else:
        print(f"Warning: Screenshot not found at {args.input}")
    ...
    canvas.convert("RGB").save(args.output, "PNG", quality=95)
